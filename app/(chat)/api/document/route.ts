...
export async function GET(request: Request) {
  ...
  if (!session?.user) {
    return new IrisError("unauthorized:document").toResponse();
  }
  ...
  if (!document) {
    return new IrisError("not_found:document").toResponse();
  }

  if (document.userId !== session.user.id) {
    return new IrisError("forbidden:document").toResponse();
  }
...
export async function DELETE(request: Request) {
  ...
  if (Number.isNaN(parsedTimestamp.getTime())) {
    return new IrisError("bad_request:api", "Invalid timestamp.").toResponse();
  }
